...
// 健康保険料率 (協会けんぽ、本人負担分 令和7年度/2025年度)
// careRateは介護保険料率（本人負担分、40歳以上の場合に加算）
const healthInsuranceRates = {
    rate: 0.05040, // 健康保険料率（大阪府の例、令和7年度も同率で据え置き）
    careRate: 0.0080 // 介護保険料率（全国一律、令和7年度も同率で据え置きの仮定）
};

// 厚生年金保険料率 (本人負担分、令和7年度/2025年度)
const pensionRate = 0.0915; // 18.3% の折半（令和7年度も同率で据え置き）

// 国民年金保険料 (月額、令和7年度/2025年度)
const kokunenPremiumMonthly = 17510; // 令和7年度の金額

// 国民健康保険料モデルデータ (令和7年度/2025年度の参考値)
// 大阪市が公表している令和6年度（2024年度）国民健康保険料率を参考としたモデル
const kokuhoModel = {
    baseKojo: 430000, // 基礎控除額（住民税の算出時）
    medical: {
        incomeRate: 0.0683, // 所得割率 (大阪市の例)
        perMember: 33000,  // 均等割額 (大阪市の例)
        limit: 670000      // 賦課限度額 (大阪市の例)
    },
    support: { // 後期高齢者支援金分
        incomeRate: 0.0239, // 所得割率 (大阪市の例)
        perMember: 11000,  // 均等割額 (大阪市の例)
        limit: 220000      // 賦課限度額 (大阪市の例)
    },
    care: { // 介護分 (40歳以上の場合)
        incomeRate: 0.0205, // 所得割率 (大阪市の例)
        perMember: 9000,   // 均等割額 (大阪市の例)
        limit: 170000      // 賦課限度額 (大阪市の例)
    }
};
